...
public class FindTheDifferenceOfTwoArrays {
    ...
    class Solution {
        public List<List<Integer>> findDifference(int[] nums1, int[] nums2) {
            ...
            for (int each : nums1) set1.add(each);
            for (int each : nums2) set2.add(each);
            for (Integer each : set1) if (!set2.contains(each)) l1.add(each);
            for (Integer each : set2) if (!set1.contains(each)) l2.add(each);
            ans.add(l1);
            ans.add(l2);
            return ans;
        }
